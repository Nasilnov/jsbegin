...
const player = {
    ...
    getNextStepPoint() {
        ...
        switch (this.direction) {
            case 'up':
                point.y--;
                point.y = point.y < 0 ? (settings.rowCount - 1) : point.y;
                break;
            case 'right':
                point.x++;
                point.x = point.x > ( settings.colCount -1 ) ? 0 : point.x;
                break;
            case 'down':
                point.y++;
                point.y = point.y > ( settings.rowCount - 1 ) ? 0 : point.y;
                break;
            case 'left':
                point.x--;
                point.x = point.x < 0 ? (settings.colCount -1) : point.x;
                break;
        }
